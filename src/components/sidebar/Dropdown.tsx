...
interface DropdownProps {
  ...
  const { state, dispatch, workspace_id, folder_id } = useAppState();
  ...
  const router = useRouter();
    ...
    const folderTitle: string | undefined = useMemo(() => {
      ...
      }, [state, listType, workspace_id, id, title]);
      //fileItitle
      ...
      const navigatePage = (accordionId: string, type: string) => {
        if (type === 'folder') {
          router.push(`/dashboard/${workspace_id}/${accordionId}`);
        }
        if (type === 'file'){
          router.push(`/dashboard/${workspace_id}/${folder_id}/${accordionId.split('folder')[1]}`)
        }
      };
    ...
    if (!user?.email || !workspace_id) return;
    const pathId = id.split('folder');
    if (listType === 'folder') {
      dispatch({    //optimistic -  updating workspace  before updatin db
        type: 'UPDATE_FOLDER',
        payload: {
    ...
    const addNewFile = async () => {
      ...
      dispatch({
        type: 'ADD_FILE',
        payload: { file: newFile, folder_id: id, workspace_id },
      });
      ...
      <AccordionTrigger id={listType} className='hover:no-underline p-2 dark:text-muted-foreground  text-sm' disabled={listType==='file'}>
        <div className={groupIdentifies}>
          <div className='flex gap-4 items-center justify-center overflow-hidden'>
            ...
            <input type='text' value={listType==='folder' ? folderTitle : fileTitle} className={clsx(
